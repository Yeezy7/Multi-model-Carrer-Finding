...
if __name__ == "__main__":
    # get_weather("Beijing")
    # print(get_attraction("Beijing", "sunny"))
    ...
    # --- 2. 初始化 ---
    user_prompt = "你好，请帮我查询一下今天郑州市的天气，然后根据天气推荐一个合适的旅游景点。"
    ...
    for i in range(5):
        ...
        if not action_match:
            observation = "错误：未能解析到Action，请确保你的回复严格遵循 'Thought: ... Action: ...' 的格式。"
        ...
        kwargs = dict(re.findall(r'(\w+)\s*=\s*"([^"]*)"', args_str)) # 解析参数为字典

        if tool_name in available_tools:
            observation = available_tools[tool_name](**kwargs)
        else:
            observation = f"错误：未知工具 '{tool_name}'，请检查工具名称是否正确。"
