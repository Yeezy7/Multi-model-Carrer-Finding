...
| 要素 | 说明 | 示例 |
|------|------|------|
| `example_prompt` | 每条示例的格式模板 | `PromptTemplate.from_template("单词：{word}, 反义词：{antonym}")` |
| `examples` | 示例数据列表 | `[{"word": "大", "antonym": "小"}, ...]` |
| `prefix` | 提示词前缀，任务描述 | `"告知我单词的反义词，如下示例："` |
| `suffix` | 提示词后缀，当前问题 | `"基于前面的示例，{input_word}的反义词是？"` |
| `input_variables` | 输入变量列表 | `["input_word"]` |
...
| 模板类型 | 输出类型 | 角色支持 | 变量注入方式 | 适用模型 |
|---------|---------|---------|-------------|---------|
| PromptTemplate | StringPromptValue | 无 | `.format()` 或 `.invoke()` | LLM |
| FewShotPromptTemplate | StringPromptValue | 无（通过 prefix/suffix 间接实现） | `.invoke()` | LLM |
| ChatPromptTemplate | ChatPromptValue | 原生支持多角色 | `.invoke({"role": msg})` | ChatModel |
